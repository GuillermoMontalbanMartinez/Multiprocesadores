...
int** reservarMatriz(int fila, int columna){
	int** matriz = 0;
	int* matriz_container = malloc(sizeof(int)*fila*columna);//Gran vector contiguo
	if(matriz_container){
		matriz = malloc(sizeof(int*)*fila);//Aqui guardaremos el comienzo de cada fila
		if(matriz){
...
void llenarMatrizAleatoria(int filas, int columnas, int** matriz, int min, int max){
	int diff = max-min+1;
...
void mostrarMatriz(int fila, int columna, int** matriz){
